...
export default function CompanyProfileScreen({ navigation }) {
  ...
  const handleEditProfileImage = async () => {
    try {
      ...
      if (!result.success) {
        MessageService.showError('Permission Needed', result.error || 'Unable to select company profile picture.');
        return;
      }

      const updateResult = await updateUserProfile({
        profileImageUri: result.uri,
        companyLogoUri: result.uri,
      });

      if (updateResult.success) {
        MessageService.showSuccess('Profile Updated', 'Company profile picture has been updated.');
      } else {
        MessageService.showError('Update Failed', updateResult.error || 'Unable to save company profile picture.');
      }
    } catch (error) {
      MessageService.showError('Update Failed', 'Unable to update company profile picture.');
    }
  };
      ...
      <View style={styles.profileGradient}>
          <View style={styles.profileInfo}>
            <View style={styles.avatarContainer}>
              {userData?.companyLogoUri || userData?.profileImageUri ? (
                <Image
                  source={{ uri: userData?.companyLogoUri || userData?.profileImageUri }}
